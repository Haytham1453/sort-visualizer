...
root = Tk() 
root.title('Bubble & Insert Sort Visualizer (Egi & Haviza)')
...
def StartSortingAlgorithm():
    ...
    if algorithmList.get() == 'Bubble Sort':
...
algorithmList = ttk.Combobox(UserInterfaceFrame, textvariable = selectedAlgorithm, values=['Bubble Sort', 'Insertion Sort'])
...
sortingSpeedScale = Scale(UserInterfaceFrame, from_=0, to=5.0, length=200, digits=2, resolution=0.1, orient=HORIZONTAL, label="Kecepatan Sorting [s]")
sortingSpeedScale.grid(row=0, column=2, padx=5, pady=5)

# Button to Start Sorting Algorithm
Button(UserInterfaceFrame, text="Mulai Sorting", command=StartSortingAlgorithm, bg='white').grid(row=0, column=3, padx=5, pady=5)
...
arraySizeScale = Scale(UserInterfaceFrame, from_ = 3, to=100, resolution=1, orient=HORIZONTAL, label="Ukuran Array")
...
minValueScale = Scale(UserInterfaceFrame, from_=0, to=100, resolution=1, orient=HORIZONTAL, label="Min Value")
...
Button(UserInterfaceFrame, text="Buat Array Acak", command=GenerateNewArray, bg='white').grid(row=1, column=3, padx=5, pady=5)
